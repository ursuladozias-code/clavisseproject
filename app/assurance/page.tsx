...
function getEmojiType(type: string) {
  const key = Object.keys(typeAssuranceEmoji).find((k) =>
...
function KpiCard({
  ...
  return (
    <div
      className="rounded-3xl p-4 text-white flex flex-col gap-1 shadow-sm"
...
export default function AssurancesPage() {
  ...
  const [assuranceSelectionnee, setAssuranceSelectionnee] =
    useState<Assurance | null>(null);
  ...
  const totalActives = biens.reduce(
    (s, b) => s + b.assurances.filter((a) => a.statut === "Actif").length,
    0,
  );
  const totalExpirees = biens.reduce(
    (s, b) => s + b.assurances.filter((a) => a.statut === "Expiré").length,
    0,
  );
  const biensSansAssurance = biens.filter(
    (b) => b.assurances.length === 0,
  ).length;

  const biensFiltres = biens.filter(
    (b) =>
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5">
        {/* ── HEADER ── */}
        <header
          className="-mx-4 sm:-mx-6 px-4 sm:px-6 py-5 mb-5"
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
              <div>
                ...
                <p
                  ...
                  style={{ color: "#64748b" }}
                >
                  Suivez les contrats, échéances et documents associés à vos
                  biens.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <KpiCard
                emoji="⚠️"
                value={totalExpirees}
                label="Expirés / À renouveler"
                gradient="linear-gradient(135deg, #f43f5e, #fb7185)"
              />
              <KpiCard
                emoji="🏠"
                value={biensSansAssurance}
                label="Biens sans assurance"
        ...
        {/* ── BARRE RECHERCHE ── */}
        <div
          className="bg-white rounded-3xl p-4 mb-5 shadow-sm"
        ...
        >
          ...
          <div className="hidden sm:grid grid-cols-12 gap-4 px-6 py-4 border-b border-gray-100">
            {["RÉFÉRENCE", "BIEN", "STATUT", "ASSURANCES", "ACTION"].map(
              (h) => (
                <div
                  key={h}
                  className={`text-xs font-extrabold text-gray-400 uppercase tracking-wider ${h === "BIEN" ? "col-span-4" : h === "ACTION" ? "col-span-2 text-right" : "col-span-2"}`}
                >
                  {h}
                </div>
              ),
          ...
          <div className="divide-y divide-gray-50">
            {biensFiltres.map((bien) => {
              ...
              const nbActifs = bien.assurances.filter(
                (a) => a.statut === "Actif",
              ).length;
              const nbExpires = bien.assurances.filter(
                (a) => a.statut !== "Actif",
              ).length;
              ...
              return (
                <div
                  ...
                  className="flex flex-col sm:grid sm:grid-cols-12 gap-3 sm:gap-4 px-4 sm:px-6 py-4 hover:bg-[#fff7ed] transition-all cursor-pointer group"
                >
                  {/* Référence */}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <span
              className="px-3 py-1 rounded-full text-xs font-extrabold hidden sm:inline-block"
              style={{ backgroundColor: cfg.bg, color: cfg.color }}
            >
              {cfg.label}
            </span>
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 rounded-2xl text-white text-sm font-extrabold shadow-lg transition hover:opacity-90 active:scale-95"
        ...
        {bien.assurances.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-sm p-12 text-center">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Ajoutez votre premier contrat d'assurance pour ce bien
            </p>
            <button
              onClick={onAjouterAssurance}
          ...
          <div className="grid gap-4">
            {bien.assurances.map((assurance) => {
              ...
              return (
                <div
                  key={assurance.id}
                  className="bg-white rounded-3xl p-5 sm:p-6 hover:shadow-md transition-all cursor-pointer"
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {assurance.assureur} · {assurance.numeroContrat}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 sm:gap-6 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-extrabold text-gray-700">
                          {assurance.documents.length} fichier
                          {assurance.documents.length > 1 ? "s" : ""}
                        </p>
                      </div>
                    </div>
        ...
        <div className="px-6 pt-6 pb-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-extrabold text-gray-800">
              🛡️ Ajouter une assurance
            </h2>
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-xl flex items-center justify-center text-gray-400 hover:bg-gray-100 font-bold transition"
            >
              ✕
            </button>
          </div>
            ...
            {etapes.map((label, i) => {
              ...
              return (
                <div key={label} className="flex items-center gap-2 flex-1">
                  <div
                    className={`flex items-center gap-2 flex-1 ${i > 0 ? "ml-2" : ""}`}
                  >
                    {i > 0 && (
                      <div
                        className="h-px flex-1"
                        style={{
                          backgroundColor: fait ? "#f97316" : "#e5e7eb",
                        }}
                      />
                    )}
                    <div className="flex items-center gap-1.5">
                      <div
                        className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-extrabold transition"
                        style={{
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={form.assureur}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, assureur: e.target.value }))
                  }
                  className="w-full px-4 py-3 rounded-2xl text-sm font-semibold outline-none border-2 transition"
                  style={{
                    backgroundColor: "#f8fafc",
                    borderColor: form.assureur ? "#f97316" : "#e5e7eb",
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, numeroContrat: e.target.value }))
                  }
                  className="w-full px-4 py-3 rounded-2xl text-sm font-semibold outline-none border-2 transition"
                  style={{
                    backgroundColor: "#f8fafc",
                    borderColor: form.numeroContrat ? "#f97316" : "#e5e7eb",
                  }}
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    style={{
                      backgroundColor: "#f8fafc",
                      borderColor: form.dateDebut ? "#f97316" : "#e5e7eb",
                    }}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    style={{
                      backgroundColor: "#f8fafc",
                      borderColor: form.dateEcheance ? "#f97316" : "#e5e7eb",
                    }}
                  />
                </div>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, montantPrime: e.target.value }))
                  }
                  className="w-full px-4 py-3 rounded-2xl text-sm font-semibold outline-none border-2 transition"
                  style={{
                    backgroundColor: "#f8fafc",
                    borderColor: form.montantPrime ? "#f97316" : "#e5e7eb",
                  }}
                />
              </div>
            </div>
          ...
          {etape === 3 && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-400 text-center">
                Vous pourrez ajouter des documents ultérieurement depuis la
                fiche de l'assurance.
              </p>
            </div>
          )}
